  ...
  late final resizeObserver = web.ResizeObserver(
    ((JSArray<web.ResizeObserverEntry> entries, web.ResizeObserver observer) {
      if (widget.height == null) {
        final entry = entries.toDart.lastOrNull;
        if (entry == null) return;
        final newHeight = entry.contentRect.height.toDouble();
        if ((newHeight - height).abs() < 1) return;
        _timer?.cancel();
        _timer = Timer(const Duration(milliseconds: 100), () {
          setState(() {
            height = newHeight;
            _divElement.style.height = '${newHeight}px';
          });
        });
      }
    }).toJS,
